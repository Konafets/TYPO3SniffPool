...
class TYPO3SniffPool_Sniffs_Commenting_ClassCommentSniff implements PHP_CodeSniffer_Sniff
{
    ...
    {
        return array(T_DOC_COMMENT_OPEN_TAG, T_CLASS, T_INTERFACE);
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        if (
            ...
            do {
                $prevLine = $tokens[$stackPtr - $i]['line'];
                $i++;
            }
            while ($prevLine !== ($stackPtrLine - 1));

            if ($tokens[$stackPtr - ($i - 1)]['code'] !== T_DOC_COMMENT_CLOSE_TAG) {
